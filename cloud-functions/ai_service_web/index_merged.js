...
async function _handleAction(event, context) {
  ...
  try {
    ...
    switch (action) {
      ...
      // ========== 识别创建模式（独立新增，不影响其他功能）==========
      case 'recognition_extract_all':
        return await recognitionExtractAll(event)
      // ================================================================
...
2. 按照下方的【两步提取法】，识别各模块下的经历条目并提取内容。
...
◆ 第一步：先数清每个模块下有几个独立经历
  ...
  识别"新经历开始"的信号（必须同时出现以下组合，才说明一个新的独立条目开始）：
  → 出现新的【公司名/机构名/单位名/项目名】+ 对应的【时间段或职位名】
  → 两者相互印证，缺一不可

  ⚠️ 以下内容绝对不是新经历的开始信号，不能触发新条目：
  - "项目介绍：""负责功能：""涉及技术：""项目成果：""项目背景：" 等小标题行
    → 这些是当前条目内部的结构性描述，属于同一条目，必须合并进同一个 originalText
  - 序号列表（1. 2. 3. 或 • - ※）
    → 这些是描述段落内部的分点，属于同一条目，必须合并
  - 无时间段对应的单独描述段落
    → 没有新时间段就没有新条目

  ⚠️ 只要在同一模块下出现了第二个"公司/项目名+时间段"组合，才必须生成第二个数组元素！

  示例A（2段实习 → 2个元素）：
  > [5] 南方电视台                       ← 公司名1 + 有对应时间，第1段实习开始
  > [6] 新闻实习生  2020-01~2020-05      ← 职位+时间1（印证上面是第1段实习）
  > [7] 在南方网新闻中心实习，参与...    ← 第1段实习的描述内容
  > [8] 华侨新闻                         ← 公司名2 + 有对应时间，第2段实习开始
  > [9] 编辑实习生  2019-07~2019-12      ← 职位+时间2（印证是第2段实习）
  > [10] 在华侨新闻实习，主要负责...     ← 第2段实习的描述内容
  → internship 有 2 个元素

  示例B（1个项目含多个小标题 → 1个元素，全部合并）：
  > [20] 外卖点餐平台测试项目            ← 项目名，第1个项目开始
  > [21] 项目介绍：参与外卖点餐平台...  ← 属于该项目的描述（不是新项目！）
  > [22] 负责功能：运用等价类边界值...  ← 属于该项目的描述（不是新项目！）
  > [23] 涉及技术：Springboot、Redis...  ← 属于该项目的描述（不是新项目！）
  → project 只有 1 个元素，originalText = [21]+[22]+[23] 全部合并，contentFormat = "subtitle"

◆ 第二步：对第一步识别出的每个独立经历，分别提取其描述内容

  - 每个独立经历生成一个数组元素
  - originalText 只包含该条目的【描述性文字内容】（含小标题行、序号段落、分点段落等所有描述）
  - 跳过（不写入 originalText）：公司名/学校名独行、职位名独行、纯时间段行、模块标题行
  - 同一条目内有多个描述段落（含小标题行）时，用 \\n 连接，全部合并进同一个 originalText
  - originalText 必须与原文逐字完全一致，严禁修改、润色或截断
...
【其他规则】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
- sectionOrder 表示同一模块内第几个经历条目（从0开始）
...
■ "paragraph"（自然段落格式）——以下任意一种情况必须设为 "paragraph"：
   ...
   ⚠️ 判断原则：有疑问时一律设为 "paragraph"

严格按以下JSON格式返回，不要有任何解释文字，只返回JSON：
{
  "name": "姓名",
  "work": [
    {"originalText": "第一份工作的描述内容", "sectionOrder": 0, "contentFormat": "paragraph"},
    {"originalText": "第二份工作的描述内容（不同公司则必须单独列出）", "sectionOrder": 1, "contentFormat": "paragraph"}
  ],
  "internship": [
    {"originalText": "第一段实习的描述内容（仅此实习的文字，不含第二段）", "sectionOrder": 0, "contentFormat": "paragraph"},
    {"originalText": "第二段实习的描述内容（不同公司则必须单独列出，不能与第一段合并）", "sectionOrder": 1, "contentFormat": "paragraph"}
  ],
  "project": [
    {"originalText": "第一个项目的描述内容", "sectionOrder": 0, "contentFormat": "subtitle"},
    {"originalText": "第二个项目的描述内容（不同项目必须单独列出）", "sectionOrder": 1, "contentFormat": "paragraph"}
  ],
  "school_experience": [{"originalText": "在校经历描述内容", "sectionOrder": 0, "contentFormat": "paragraph"}],
  "self_evaluation": [{"originalText": "自我评价全部内容（多段合并）", "sectionOrder": 0, "contentFormat": "paragraph"}]
}`
...
async function enhancePolishSection(event) {
  try {
    const {
      ...
      contentFormat = 'paragraph' // 'paragraph' | 'subtitle' | 'subtitle_compact'
    } = event
    if (!originalText) throw new Error('originalText 参数缺失')

    // subtitle_compact（有序排列）与 subtitle（靠左对齐）的 AI 生成内容完全相同，
    // 区别仅在回填时的段落布局，因此 AI 侧统一当作 'subtitle' 处理。
    const effectiveContentFormat = contentFormat === 'subtitle_compact' ? 'subtitle' : contentFormat
    ...
    // 自我评价不受此影响（始终使用自然段）。
    const wc = (effectiveContentFormat === 'subtitle' && moduleType !== 'self_evaluation')
...
- 此为最高优先级约束，内容质量服从字数限制`
    ...
    // ── 小标题格式分支（effectiveContentFormat === 'subtitle'，含 subtitle_compact）──────
    // AI 自由定义小标题（岗位润色：与岗位核心能力对应；自身润色：提炼内容结构）
    // 输出格式为 "小标题：内容\n小标题：内容"，分隔符必须使用全角冒号"："
    if (effectiveContentFormat === 'subtitle') {
...
/**
 * 仅保留 moduleSpecs 中出现的模块 key（标识库决定的采集清单白名单）
 */
function whitelistExtractedToSpecs(extractedData, moduleSpecs) {
  if (!extractedData || typeof extractedData !== 'object') return {}
  const allowed = new Set(moduleSpecs.map(s => s.moduleKey))
  const out = {}
  allowed.forEach((key) => {
    if (Object.prototype.hasOwnProperty.call(extractedData, key)) {
      out[key] = extractedData[key]
    }
  })
  return out
}

/** 按记录库 maxCount 截断列表型模块（与前端 normalize 一致，服务端再兜底） */
function clampExtractedArraysToSpecs(extractedData, moduleSpecs) {
  ...
  for (const spec of moduleSpecs) {
    const { moduleKey, maxCount } = spec
    if (!ARRAY_KEYS.has(moduleKey) || !maxCount || maxCount <= 0) continue
    const raw = out[moduleKey]
    if (!Array.isArray(raw) || raw.length <= maxCount) continue
    out[moduleKey] = raw.slice(0, maxCount)
  }
  return out
}

/**
 * 识别模式 — 模板清单驱动的「按需采集」，非整份旧简历全量抽取
 *
 * 输入：
 *   textContent  string    旧简历全文（仅作素材池）
 *   userType     string    'work'|'internship'|'student'（与所选模板组合）
 *   moduleSpecs  Array     系统根据【标识库 + 记录库】为当前模板预构建的唯一数据清单：
 *                          [{ moduleKey, label, maxCount }]
 *                          由 buildModuleSpecList(templateId, userType) 生成
 *   templateId   string    可选，如 word_r_1，写入提示词便于模型理解目标模板
 *
 * 输出：{ success: true, extractedData }
 *       extractedData 仅含清单内模块 key，且列表条数不超过各模块 maxCount
 */
async function recognitionExtractAll(event) {
  const { textContent, userType, moduleSpecs, templateId = '' } = event

  if (!textContent || !textContent.trim()) {
    return { success: false, message: '简历内容为空，请提供有效的简历文本' }
  }
  if (!moduleSpecs || !moduleSpecs.length) {
    return { success: false, message: '缺少模块规格（moduleSpecs），请先选择简历模板' }
  }

  const userTypeDesc = {
    work:        '全职求职，有工作经验',
    internship:  '实习/应届生，在校或刚毕业',
    student:     '在校学生，以校内经历为主',
  }[userType] || '身份未知'

  // ── 根据模块规格动态构建字段说明 ──────────────────────────────────────────
  const MODULE_FIELD_DEFS = {
    // 与简历定制/网页识别一致：基本信息仅提取四项即可，勿展开其它字段以免干扰缺口判断
    basic: `basic（基本信息，对象，仅以下 4 个字段；未识别填 ""）：
   name（姓名）、gender（性别）、phone（手机号）、email（邮箱）`,

    career: `career（求职意愿，对象）：
   position（期望职位）、salary（期望薪资）、city（期望城市）
   status（求职状态，限定值：随时到岗/求职中/看机会/已就业）`,

    education: `education（教育经历，数组，最多{maxCount}条）：
   每条：school（学校）、college（学院）、major（专业）
   degree（学历：本科/硕士/博士/专科/高中及以下）
   startDate（YYYY.MM）、endDate（YYYY.MM）、courses（主修课程）、gpa（成绩）`,

    work: `work（工作经历，数组，最多{maxCount}条，按时间倒序）：
   每条：company（公司）、department（部门）、position（职位）
   startDate（YYYY.MM）、endDate（YYYY.MM，在职填"至今"）
   content（工作内容，完整逐字保留原文，禁止任何修改）`,

    internship: `internship（实习经历，数组，最多{maxCount}条，按时间倒序）：
   每条：company（公司）、position（职位）
   startDate（YYYY.MM）、endDate（YYYY.MM）
   content（实习内容，完整逐字保留原文，禁止任何修改）`,

    project: `project（项目经历，数组，最多{maxCount}条）：
   每条：projectName（项目名称）、role（担任角色）
   startDate（YYYY.MM）、endDate（YYYY.MM）
   content（项目内容，完整逐字保留原文，禁止任何修改）`,

    school_experience: `school_experience（在校经历，数组，最多{maxCount}条）：
   每条：experienceName（经历名称）、role（担任角色）
   startDate（YYYY.MM）、endDate（YYYY.MM）
   content（经历内容，完整逐字保留原文，禁止任何修改）`,

    self_evaluation: `self_evaluation（自我评价，对象）：
   content（完整保留原文，禁止任何修改）`,

    skill: `skill（职业技能，数组，最多{maxCount}条）：
   每条：skillName（技能名称）、level（掌握程度，限定值：入门/初级/熟练/精通/专家，未注明默认"熟练"）`,

    certificate: `certificate（奖项证书，数组，最多{maxCount}条）：
   每条：name（证书/奖项名称）、date（获得时间，YYYY.MM）、issuer（颁发机构）`,

    skill_certificate: `skill_certificate（技能证书/资格证书，数组，最多{maxCount}条）：
   每条：name（证书名称）、date（获得时间，YYYY.MM）、issuer（颁发机构）`,

    hobby: `hobby（兴趣爱好，数组，最多{maxCount}条）：
   每条：name（爱好名称）`,

    exam_info: `exam_info（报考信息，对象）：
   school（报考学校）、major（报考专业）、totalScore（总分）
   subjects（科目数组：[{ name, score }]）`,
  }

  // 只为 moduleSpecs 中指定的模块生成字段说明
  const moduleDescLines = moduleSpecs.map((spec, idx) => {
    const def = MODULE_FIELD_DEFS[spec.moduleKey] || `${spec.moduleKey}（${spec.label}，详细提取所有可用信息）`
    return `${idx + 1}. ${def.replace('{maxCount}', spec.maxCount)}`
  })

  // 需要保留原文的模块
  const preserveModules = moduleSpecs
    .filter(s => ['work','internship','project','school_experience','self_evaluation'].includes(s.moduleKey))
    .map(s => s.label)

  const arrayModuleKeys = ['work', 'internship', 'project', 'school_experience']
  const needsArrayRules = moduleSpecs.some(s => arrayModuleKeys.includes(s.moduleKey))

  const arrayExtractionBlock = needsArrayRules ? `

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【经历类数组模块：两步提取法（与简历优化-模块识别对齐，必须遵守）】
适用于本任务中出现的：work / internship / project / school_experience 数组。

◆ 第一步：先数清该模块下有几个「独立经历/条目」
  只有同时出现新的【公司名/机构名/单位名/项目名/经历名】+ 对应的【时间段或职位/角色】相互印证时，才算新的一条数组元素。
  ⚠️ 以下内容绝对不是新条目的开始，不得拆成多条：
  - 「项目介绍：」「负责功能：」「涉及技术：」「项目成果：」「项目背景：」等小标题行 → 属于当前条目内部，须合并进同一条的 content
  - 序号列表（1. 2. 3. 或 • - ※）→ 条目内分点，须合并进同一条的 content
  - 无新时间段、无新项目名对应的单独描述 → 不得新建数组元素

  示例（1 个项目含多个小标题 → 只能 1 条 project）：
  「外卖点餐平台」+ 时间 为一项；其后的「项目介绍：…」「负责功能：…」全部写入该条的 content，禁止拆成第 2 条 project。

◆ 第二步：对每条经历分别填字段
  - content（及同类长文本）：与原文逐字一致，禁止润色、删减、合并改写
  - 条数不得超过各模块标明的「最多N条」；超过时按时间倒序只保留最新的 N 条（与下列规则 4 一致）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
` : ''

  const templateHint = templateId
    ? `【目标模板】用户所选简历模板标识为：${templateId}（与求职者身份组合后，唯一确定下方采集清单；旧简历中凡不属于该清单所需的信息，一律不采集、不输出）。\n\n`
    : ''

  const checklistSummary = moduleSpecs
    .map(s => `- ${s.label}（${s.moduleKey}）：列表模块最多 ${s.maxCount} 条；对象模块按字段说明填写`)
    .join('\n')

  const prompt = `你是专业简历解析专家，擅长从各类简历文本中精确提取结构化信息。

【求职者身份】${userTypeDesc}

${templateHint}【核心原则：模板清单驱动采集 — 与「整份简历全量抓取」无关】
1. 下方「需要提取的模块及字段要求」= 系统根据用户所选简历模板，依据【模块标识库（要哪些模块、顺序）】与【模块记录库（各模块条数上限）】预先构建的唯一数据清单。你的输出必须且只能服务于填这份目标模板，而不是备份整份旧简历。
2. 旧简历全文仅是素材来源。你只能从原文中摘取能对应到清单项下的内容；无法对应到清单任一模块/字段的段落，直接忽略，禁止为其新建模块或塞进清单外的 key。
3. 清单中没有的模块（例如目标模板不含「工作经历」而旧简历里有），绝对不要输出该 key，也不要把工作经历改写后填入其它模块。
4. 用户后续在客户端上的「补全」也严格按同一份清单执行；你只需为清单内字段填空，不要扩展清单、不要臆造清单外的经历条数。

【本模板采集清单摘要（模块 + 条数上限）】
${checklistSummary}

【需要提取的模块及字段要求（逐项对照旧简历采集）】
${moduleDescLines.join('\n\n')}
${arrayExtractionBlock}
【提取规则】
1. JSON 中只包含以上 ${moduleSpecs.length} 个模块的 key，与清单顺序无关但 key 必须完全一致；禁止添加任何清单外的 key
2. ${preserveModules.length > 0 ? `"${preserveModules.join('、')}"的 content 字段：必须逐字完整保留原文，禁止修改、润色、删减任何内容` : '所有字段按格式要求提取'}
3. 时间格式统一转换为 YYYY.MM（如"2023年3月" → "2023.03"，"23.03" → "2023.03"）
4. 数组模块：条数不得超过各模块标明的最多条数；超出则按时间倒序只保留最新的 N 条；且同一项目/实习不得因小标题或分点拆成多条（见上「两步提取法」）
5. 未识别到的字段填 "" 空字符串，未识别到的整个模块（数组类型）填 []，对象类型填 null
6. basic 对象仅包含 name、gender、phone、email 四个字段，不要输出其它基本信息字段
7. 直接输出 JSON 对象，不添加 markdown 代码块，不添加任何解释文字

【旧简历原文（素材池，按需摘取而非全量照搬）】
${textContent}`

  try {
    // 与 enhance_extract_sections 一致：提取专版（temperature 0 + 忠实系统提示）
    const rawText = await callDeepSeekForExtraction(prompt)
    ...
    try {
      ...
      console.error('recognition_extract_all JSON解析失败:', parseErr.message)
      console.error('AI原始返回(前500字):', rawText.slice(0, 500))
      return { success: false, message: 'AI返回格式异常，请重试' }
    }

    let extractedDataOut = whitelistExtractedToSpecs(extractedData, moduleSpecs)
    extractedDataOut = clampExtractedArraysToSpecs(extractedDataOut, moduleSpecs)

    return { success: true, extractedData: extractedDataOut }
  } catch (err) {
    console.error('recognition_extract_all 调用失败:', err.message)
    return { success: false, message: err.message || 'AI识别失败' }
  }
}

/**
 * 调用 DeepSeek API 返回原始文本（识别模式专用，不做额外处理）
 */
async function callDeepSeekAPIRaw(userPrompt) {
  const response = await axios.post(
    `${DEEPSEEK_CONFIG.baseURL}/v1/chat/completions`,
    {
      ...
      messages: [
        {
          role:    'system',
          content: '你是专业简历解析专家。请严格按照用户要求提取简历信息，直接输出JSON，不添加任何额外说明。'
        },
...
// ── 小程序端免鉴权白名单（无需 JWT 即可调用）──────────────────────────────────
// 仅限简历定制功能使用：小程序直接调用 AI 识别，没有 JWT 登录令牌
const MINIPROGRAM_PUBLIC_ACTIONS = new Set([
  'recognition_extract_all',  // 简历定制：从文字段落中提取结构化简历数据
])
...
exports.main = async (event, context) => {
  ...
  if (MINIPROGRAM_PUBLIC_ACTIONS.has(body.action)) {
    try {
      ...
      console.error(`ai_service_web [public] ${body.action} error:`, err);
