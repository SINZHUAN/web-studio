...
const config = {
  ...
  bucket: process.env.TENCENT_BUCKET   //存储池地址为腾讯云数据万象存储桶地址，非云开发存储地址
}
...
async function downloadTemplate(event) {
  try {
    ...
    const cloudFileId = `cloud://cloud2-2giceoll88a02-3b3b3dcd374.636c-cloud2-2giceoll88a02-3b3b3dcd374-1340085102/${templatePath}`
...
async function docToImage(event) {
  try {
    ...
    if (convertResult && convertResult.success) {
      // convertWordToImages 返回 { images: [signedUrl], ... }
      const previewImageUrl = (convertResult.images && convertResult.images[0]) || null
      return {
        success: true,
        previewImageUrl,
        message: '文档转图片成功'
...
async function replaceByParagraph(event) {
  try {
    ...
    const buildNewPara = (pXml, polishedText, contentFormat) => {
      ...
      // 仅当 contentFormat === 'subtitle' 且有多行时，才使用独立段落模式
      ...
      // ── 小标题独立段落模式（subtitle 格式）────────────────────────────────
      // 每个小标题行生成独立 <w:p>，继承原始段落的 <w:pPr>，标题部分加粗
      if (isSubtitleFormat) {
        // 预扫描 bodyXml，提取首个内容 run 的 <w:rPr>（不修改 bodyXml）
        let firstRunRprXml = ''
        ...
        // 每行 → 独立 <w:p>：共享 headXml（含 pPr），标题部分加粗 + 内容部分正常
